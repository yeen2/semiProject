...
<%@ include file="../includes/main/header.jsp" %>
	...
	<div class="container" style="width:550px;border: 1px solid white; height: 800px;">
		...
		<textarea class="form-control" rows="13" cols="45" name="q_content"><%=q.getQ_content() %></textarea></td>
		</tr>
		</table>
		<br>
		<hr>
<input type="hidden" name="q_no" value = <%=q.getQ_no() %>>
		...
		<button class="btn btn-primary"  name="info" value="info" style="margin-left:215px; margin-right:5px;">수정완료</button>
		<button class="btn btn-secondary" type="button" onclick="location.href='selectQuestion.qu?q_no=<%=q.getQ_no() %>&info=cancle'"><i class="fa fa-fw fa-lg fa-times-circle"></i>취소</button>
		</div>
