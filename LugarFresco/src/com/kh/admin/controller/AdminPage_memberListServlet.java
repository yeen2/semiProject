...
public class AdminPage_memberListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
  
    public AdminPage_memberListServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("kinds") != null && request.getParameter("search") != null) {
			...
			int listCount = new AdminPageService().getMemberListCount();
			...
			if (kinds.equals("memberNick")) {

				String search = request.getParameter("search");

				list = new AdminPageService().selectMemberNickNameSearch(search);

			} else {
				
				String search = request.getParameter("search");

				list = new AdminPageService().selectMemberEmailSearch(search);
			}
			...
			if(list.isEmpty()) {
				...
				request.setAttribute("msg", "검색 내용은 회원리스트에 존재하지 않습니다.");
